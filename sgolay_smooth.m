function [fit,slope,curve] = sgolay_smooth(x,Y,wid,ord)
%sgolay_smooth Smooth the response Y of the data [x,Y] using a
%Savitzky-Golay Smoothing and Differentiation Filter of width wid and order ord, where wid is
%an odd number and ord<wid, and output as well both the 1st and 2nd derivative of the fit
%default : wid=5,ord=wid-3
%minimal version : [fit,~,~] = sgolay_smooth(1:size(Y,1),Y)
%Chris version : [~,slope,curve] = sgolay_smooth(1:size(Y,1),Y)
...
if(~mod(wid,2))
    disp('Error: width must be an odd number');
    return;
end
if(ord<1 || ord > wid-1)
...
%     disp(j);
    for i=1:m(1)-wid+1
        vecy = Y(i:i+wid-1,j);
        vecx = x(i:i+wid-1);
        vecx = vecx - vecx((wid-1)/2+1);
        G = zeros(3,wid);
        G(1,:) = sgsdf(vecx,ord);
        fit(i+(wid-1)/2,j)=G(1,:)*vecy;
        if(ord > 0)
            G(2,:) = sgsdf(vecx,ord,1);
            slope(i+(wid-1)/2,j)=G(2,:)*vecy;
        end
        if(ord > 1)
            G(3,:) = sgsdf(vecx,ord,2);
            curve(i+(wid-1)/2,j)=G(3,:)*vecy;
        end
    end
end
